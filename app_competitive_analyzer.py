...
def create_price_evolution_chart(df, date_col, price_vars, client_brand):
    """Crea gráfico de evolución de precios"""
    fig = go.Figure()
    
    colors = {'client': '#1E88E5', 'Colgate': '#E53935', 'OralB': '#43A047'}
    
    for brand, col in price_vars.items():
        if col in df.columns:
            label = client_brand if brand == 'client' else brand
            color = colors.get(brand, '#FFA726')
...
def create_price_index_chart(df, date_col, client_brand):
    ...
    colors = ['#E53935', '#43A047', '#FFA726', '#AB47BC']
    ...
    fig.update_layout(
        title=f'Índice de Precio Relativo - {client_brand} vs Competencia',
        xaxis_title='Período',
...
def create_units_market_share_chart(df, date_col):
    ...
    colors = {'MS_Units_client': '#1E88E5', 'MS_Units_Colgate': '#E53935', 
              'MS_Units_OralB': '#43A047'}
    
    for col in ms_cols:
        brand = col.replace('MS_Units_', '')
        label = "Cliente" if brand == 'client' else brand
        color = colors.get(col, '#FFA726')
...
def create_value_comparison_chart(df, date_col, value_vars, client_brand):
    ...
    for brand, col in value_vars.items():
        if col in df.columns:
            label = client_brand if brand == 'client' else brand
            color = colors.get(brand, '#FFA726')
...
def create_distribution_analysis_chart(df, date_col, dist_vars, client_brand):
    ...
    for brand, col in dist_vars.items():
        if col in df.columns:
            label = client_brand if brand == 'client' else brand
            color = colors.get(brand, '#FFA726')
...
def create_google_trends_chart(df, date_col, gt_vars, client_brand):
    ...
    for brand, col in gt_vars.items():
        if col in df.columns:
            label = client_brand if brand == 'client' else brand
            color = colors.get(brand, '#FFA726')
...
def create_competitive_radar(metrics, client_brand):
...
with st.sidebar:
    ...
    if st.session_state.analyzer is not None:
        st.markdown("---")
        st.markdown("### 🏷️ Configuración de Marcas")
        
        client_brand = st.text_input(
            "Marca Cliente",
            value="Listerine",
            help="Nombre de tu marca/cliente"
        )
        
        competitors_input = st.text_input(
            "Competidores (separados por coma)",
            value="Colgate, OralB",
            help="Ej: Colgate, OralB, Sensodyne"
        )
        ...
        st.markdown("---")
        st.markdown("### 🔧 Selección Manual de Variables")
        ...
        if st.button("🚀 Ejecutar Análisis Completo", type="primary", width='stretch'):
            with st.spinner("Analizando datos..."):
                ...
                st.session_state.analyzer.configure_variables(client_brand, competitors)
                
                # Aplicar variables manuales si están seleccionadas
                ...
                if st.session_state.get('precio_comp'):
                    for i, col in enumerate(st.session_state.precio_comp):
                        comp_name = competitors[i] if i < len(competitors) else f'Comp{i+1}'
                        st.session_state.analyzer.price_vars[comp_name] = col
                ...
                if st.session_state.get('unid_comp'):
                    for i, col in enumerate(st.session_state.unid_comp):
                        comp_name = competitors[i] if i < len(competitors) else f'Comp{i+1}'
                        st.session_state.analyzer.units_vars[comp_name] = col
                ...
                if st.session_state.get('valor_comp'):
                    for i, col in enumerate(st.session_state.valor_comp):
                        comp_name = competitors[i] if i < len(competitors) else f'Comp{i+1}'
                        st.session_state.analyzer.value_vars[comp_name] = col
                ...
                if st.session_state.get('dist_comp'):
                    for i, col in enumerate(st.session_state.dist_comp):
                        comp_name = competitors[i] if i < len(competitors) else f'Comp{i+1}'
                        st.session_state.analyzer.dist_vars[comp_name] = col
                ...
                if st.session_state.get('gt_comp'):
                    for i, col in enumerate(st.session_state.gt_comp):
                        comp_name = competitors[i] if i < len(competitors) else f'Comp{i+1}'
                        st.session_state.analyzer.gt_vars[comp_name] = col
...
if st.session_state.analyzer is not None and st.session_state.analysis_complete:
    ...
    st.markdown(f"**Análisis Competitivo Multi-Dimensional** | {client_brand} vs Mercado")
    ...
    with tab1:
        ...
        with col1:
            radar_fig = create_competitive_radar(metrics, client_brand)
        ...
        if 'precios' in metrics:
            ...
            if analyzer.price_vars:
                fig_price_evol = create_price_evolution_chart(
                    df, analyzer.date_column, analyzer.price_vars, client_brand
                )
            ...
            index_cols = [col for col in df.columns if 'PriceIndex_' in col]
            if index_cols:
                fig_price_index = create_price_index_chart(df, analyzer.date_column, client_brand)
        ...
        if 'valor' in metrics:
            ...
            if analyzer.value_vars:
                fig_value = create_value_comparison_chart(
                    df, analyzer.date_column, analyzer.value_vars, client_brand
                )
        ...
        if 'distribucion' in metrics:
            ...
            if analyzer.dist_vars:
                fig_dist = create_distribution_analysis_chart(
                    df, analyzer.date_column, analyzer.dist_vars, client_brand
                )
        ...
        if 'google_trends' in metrics:
            ...
            if analyzer.gt_vars:
                fig_gt = create_google_trends_chart(
                    df, analyzer.date_column, analyzer.gt_vars, client_brand
                )
    ...
    with col2:
        ...
        2. **Configura** tu marca y competidores
