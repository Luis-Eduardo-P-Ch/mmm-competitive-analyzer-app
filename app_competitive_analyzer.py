...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        with st.expander("👁️ Preview de datos"):
        ...
        client_brand = st.text_input(
            "Nombre de tu marca (Cliente)",
            value="LIST",
            help="Ej: LIST, Listerine, etc."
        )
        ...
        competitors_input = st.text_area(
            ...
            value="Colgate\nOralB",
            help="Ingresa cada competidor en una línea separada"
        )
        
        competitors = [c.strip() for c in competitors_input.split('\n') if c.strip()]
        
        st.info(f"📊 Analizando: **{client_brand}** vs {len(competitors)} competidores")
        ...
        if st.button("🚀 Ejecutar Análisis Completo", type="primary", width='stretch'):
            with st.spinner("⏳ Procesando datos..."):
                try:
                    # Crear analizador
                    analyzer = CompetitiveAnalyzer(filepath)
                    
                    # Configurar variables
                    analyzer.configure_variables(client_brand, competitors)
        ...
        if st.session_state.analysis_complete:
            st.markdown("---")
            st.markdown("### ℹ️ Estado del Análisis")
...
if st.session_state.analysis_complete and st.session_state.analyzer is not None:
