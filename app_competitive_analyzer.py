...
def create_yearly_stats_table(df, date_col, variables_dict, metric_name):
    """
    Crea una tabla de estadísticas anuales por marca (PROMEDIO MENSUAL)
...
def create_yearly_total_table(df, date_col, variables_dict, metric_name):
    """
    Crea una tabla de SUMA TOTAL anual por marca
    ...
    date_col : str
        Nombre de la columna de fecha
    variables_dict : dict
        Diccionario con las variables por marca (ej: {'client': 'Unid_LIST', 'Colgate': 'Unid_Colgate'})
    metric_name : str
        Nombre de la métrica (para el formato de visualización)
    ...
    --------
    pd.DataFrame
        Tabla con las sumas totales anuales
    """
    # Crear copia del dataframe con año
    df_copy = df.copy()
    df_copy['Year'] = pd.to_datetime(df_copy[date_col]).dt.year
    ...
    # Calcular suma total por marca y año
    for brand, col in variables_dict.items():
        if col in df.columns:
            # Nombre de la marca para visualización
            label = f"{col}" if brand != 'client' else f"{col} (Cliente)"
            
            # Agrupar por año y calcular SUMA
            yearly_sum = df_copy.groupby('Year')[col].sum()
            
            # Suma total (todos los años)
            overall_sum = df_copy[col].sum()
            
            # Crear diccionario de datos
            row_data = {'Marca': label}
            
            # Agregar suma de cada año
            for year in sorted(yearly_sum.index):
                if metric_name in ['Precio', 'Price']:
                    row_data[f'{year}'] = f"${yearly_sum[year]:,.0f}"
                elif metric_name in ['Unidades', 'Units', 'Valor', 'Value']:
                    row_data[f'{year}'] = f"{yearly_sum[year]:,.0f}"
                elif metric_name in ['Distribución', 'Distribution']:
                    row_data[f'{year}'] = f"{yearly_sum[year]:,.0f}"
                else:
                    row_data[f'{year}'] = f"{yearly_sum[year]:,.2f}"
            
            # Agregar suma total
            if metric_name in ['Precio', 'Price']:
                row_data['Total General'] = f"${overall_sum:,.0f}"
            elif metric_name in ['Unidades', 'Units', 'Valor', 'Value']:
                row_data['Total General'] = f"{overall_sum:,.0f}"
            elif metric_name in ['Distribución', 'Distribution']:
                row_data['Total General'] = f"{overall_sum:,.0f}"
            else:
                row_data['Total General'] = f"{overall_sum:,.2f}"
...
def create_competitive_radar(metrics):
    ...
    # Distribución - Normalizar relativo al máximo de distribución
    if 'distribucion' in metrics and 'client' in metrics['distribucion']:
        client_dist = metrics['distribucion']['client']['actual']
        
        # Encontrar el máximo de distribución entre todas las marcas
        max_dist = client_dist
        for brand, stats in metrics['distribucion'].items():
            # Excluir métricas calculadas
            if brand.startswith('share_') or brand == 'fair_share':
                continue
            if isinstance(stats, dict) and 'actual' in stats:
                if stats['actual'] > max_dist:
                    max_dist = stats['actual']
        
        # Normalizar: el que tiene más distribución obtiene 100
        if max_dist > 0:
            dist_score = (client_dist / max_dist) * 100
        else:
            dist_score = 0
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        # Guardar archivo temporalmente si no está guardado
        if 'temp_file_path' not in st.session_state or st.session_state.temp_file_path is None:
            file_path = save_uploaded_file(uploaded_file)
            st.session_state.temp_file_path = file_path
        else:
            file_path = st.session_state.temp_file_path
        
        # Mostrar selector de columna de fecha si aún no se ha cargado
        if not st.session_state.get('file_loaded', False):
            # Leer columnas para seleccionar la de fecha
            temp_df = pd.read_excel(file_path)
            
            st.markdown("### 📅 Seleccionar Columna de Fecha")
            
            # Intentar detectar automáticamente la columna de fecha
            possible_date_cols = [col for col in temp_df.columns 
                                if any(keyword in col.lower() 
                                      for keyword in ['date', 'fecha', 'period', 'mes', 'month', 'año', 'year'])]
            
            default_idx = 0
            if possible_date_cols:
                default_idx = temp_df.columns.tolist().index(possible_date_cols[0])
            
            date_column = st.selectbox(
                "¿Qué columna contiene las fechas?",
                options=temp_df.columns.tolist(),
                index=default_idx,
                help="Selecciona la columna que contiene las fechas de tu dataset"
            )
            
            if st.button("✅ Confirmar y Cargar Datos", type="primary"):
    ...
    if st.session_state.analyzer is not None:
        ...
        if st.session_state.analyzer is not None:
            ...
            date_col = st.session_state.analyzer.date_column
            all_columns = [col for col in df.columns if col != date_col]
...
if st.session_state.analyzer is not None and st.session_state.analysis_complete:
    ...
    with tab2:
        ...
        if 'precios' in metrics:
            ...
            for brand, stats in metrics['precios'].items():
                # Excluir métricas calculadas que no son marcas
                if brand.startswith('index_vs_') or brand in ['positioning', 'elasticity']:
                    continue
    ...
    with tab3:
        ...
        if 'unidades' in metrics:
            ...
            for brand, stats in metrics['unidades'].items():
                # Excluir métricas calculadas que no son marcas
                if brand.startswith('ms_') or brand == 'tendencia':
                    continue
            ...
            # NUEVA TABLA: Suma total de unidades por año
            st.markdown("---")
            st.markdown("### 📊 Suma Total de Unidades por Año")
            st.markdown("*Suma de todas las unidades vendidas en cada año*")
            
            yearly_units_total = create_yearly_total_table(
                df, analyzer.date_column, analyzer.units_vars, 'Unidades'
            )
            
            if yearly_units_total is not None:
                st.dataframe(yearly_units_total, width='stretch', hide_index=True)
            else:
                st.info("No hay datos suficientes para generar la tabla de totales anuales")
            
            # TABLA: Estadísticas anuales de unidades (promedio mensual)
            st.markdown("---")
            st.markdown("### 📅 Promedio Mensual de Unidades por Año")
            st.markdown("*Promedio mensual de unidades en cada año*")
    ...
    with tab4:
        ...
        if 'valor' in metrics:
            ...
            for brand, stats in metrics['valor'].items():
                # Excluir métricas calculadas que no son marcas
                if brand.startswith('ms_') or brand.startswith('cagr_'):
                    continue
    ...
    with tab5:
        ...
        if 'distribucion' in metrics:
            ...
            dist_data = []
            # Filtrar solo las métricas que corresponden a marcas (excluir share_ y fair_share)
            for brand, stats in metrics['distribucion'].items():
                # Excluir métricas calculadas que no son marcas
                if brand.startswith('share_') or brand == 'fair_share':
                    continue
